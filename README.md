...
run_parameter_analysis.py      教材算法参数敏感性实验
results/                       输出 CSV 结果
...
这个实验的目的不是替换主算法，而是比较不同教材算法在同一模型上的表现。表中的 `objective` 是原始连续变量上的模型目标值，`recovered_objective` 是恢复成可执行方案后的目标值或选址得分；任务 2 的恢复值会先把极小的负流量修复为 0 后再计算运输成本，避免把近似不可行解和可解释结果混在一起。`equality_residual` 越小表示约束满足得越好。ADMM 和增广拉格朗日法按固定迭代预算运行，因此状态为 `max_iter` 时表示得到的是近似解，而不是严格最优解。运行时间会随机器负载略有波动，表中给出的是一次实测结果。

任务 1：AGV 任务分配。

| 算法 | 状态 | 连续目标值 | 恢复后路线成本 | 迭代步数 | 时间 | 等式残差 | 分析 |
| --- | --- | ---: | ---: | ---: | ---: | ---: | --- |
| 原始-对偶内点法 | optimal | 131.000 | 131.000 | 13 | 0.281s | 4.81e-08 | 我们实现的主算法，精度高，恢复后路线成本稳定 |
| HiGHS 对偶单纯形法 | optimal | 131.000 | 131.000 | 568 | 0.024s | 0 | 库实现非常快，可作为 LP 最优值参考 |
| ADMM | max_iter | 130.597 | 131.000 | 1200 | 3.253s | 2.31e-03 | 固定迭代预算下仍有可行性残差，所以连续目标值略低于最优值；整数恢复后仍得到成本 131 的可执行方案 |

任务 2：动态分区。

| 算法 | 状态 | 目标值 | 非零流数量 | 迭代步数 | 时间 | 等式残差 | 分析 |
| --- | --- | ---: | ---: | ---: | ---: | ---: | --- |
| 原始-对偶内点法 | optimal | 79350.2 | 403 | 21 | 0.044s | 6.05e-08 | 主算法，能够稳定得到高精度 LP 解 |
| HiGHS 对偶单纯形法 | optimal | 79350.2 | 151 | 309 | 0.007s | 7.82e-17 | 同样达到最优值；单纯形法给出的极点解更稀疏 |
| ADMM | max_iter | 83050.8 | 485 | 1500 | 0.427s | 5.95e-05 | 可行性接近满足，但目标值仍高于最优值，说明一阶分裂法收敛较慢 |
| 增广拉格朗日 + BB | max_iter | 81974.6 | 484 | 6000 | 1.180s | 7.26e-07 | 约束满足较好，目标值比 ADMM 更接近最优，但仍不如内点法和单纯形法 |

任务 3：重点缓存货位选择。

| 算法 | 状态 | 连续目标值 | 选址得分 | 迭代步数 | 时间 | 选中点数 | 最小间距 | 分析 |
| --- | --- | ---: | ---: | ---: | ---: | ---: | ---: | --- |
| 二次罚函数 + 投影 BB | optimal | -0.003164 | 5.198 | 2400 | 0.136s | 10 | 7 | 当前主算法，速度快，修复后满足间距约束 |
| 二次罚函数 + 普通投影梯度 | optimal | -0.003166 | 5.198 | 2400 | 0.259s | 10 | 7 | 结果质量相同，但比 BB 慢，说明 BB 步长有加速作用 |
| 二次罚函数 + Nesterov | optimal | -0.003171 | 5.198 | 2400 | 0.327s | 10 | 7 | 本例中加速动量没有明显优势，可能受投影和罚函数病态影响 |
| 增广拉格朗日 + 投影 BB | optimal | -0.004074 | 3.667 | 2000 | 0.092s | 10 | 7 | 连续约束满足很好，但离散修复后的选址得分较低，说明连续松弛好不等于离散方案最好 |
...
算法对比确定了“哪些方法能用”，参数敏感性分析进一步回答“同一个方法应该怎么调”。运行：
...
这个脚本只读取已经存在的 `results/algorithm_benchmark.csv` 作为基准值，不重跑前面的算法对比实验。这里选择的参数都对应教材算法中的核心调节量：

- ADMM 的 `rho`：控制线性目标、盒约束投影和等式约束投影之间的惩罚平衡；
- 增广拉格朗日法的 `rho0`：控制初始约束惩罚强度；
- 二次罚函数法的 `rho0` 与 `rho_multiplier`：控制罚因子从一开始有多强、每轮增长多快；
- 内层迭代预算保持不变，因此表中的变化主要体现参数对收敛速度和解质量的影响。

![算法参数敏感性分析](figures/algorithm_parameter_sweep.png)

主要结果如下：

| 实验 | 参数范围 | 代表性结果 | 结论 |
| --- | --- | --- | --- |
| 任务 1 ADMM | `rho = 5, 10, 25, 50, 100` | 所有设置恢复后的路线成本都是 `131`；等式残差从 `1.76e-2` 降到最低约 `1.21e-3` | 任务 1 的整数恢复很稳，`rho` 主要影响连续松弛的可行性，而不影响最终路线成本 |
| 任务 2 ADMM | `rho = 10, 20, 35, 70, 120` | `rho=10` 的运输成本差距约 `+980`，`rho=120` 增加到约 `+12432` | 固定 1500 步预算下，过大的 `rho` 会更强调约束投影，反而让目标值变差；任务 2 对 ADMM 参数很敏感 |
| 任务 2 增广拉格朗日 | `rho0 = 0.25, 0.5, 1, 2, 4` | `rho0=0.25` 成本差距约 `+1145`，但残差较大；`rho0=2` 达到 `optimal`，但成本差距约 `+5491` | 初始罚因子越大，约束满足越快，但在固定预算下不一定得到更低运输成本 |
| 任务 3 二次罚函数 | `rho0 = 1, 3, 10, 30`，`rho_multiplier = 2, 4, 8, 12` | 多数组合恢复后选址得分为 `5.198`；`rho0=1` 或 `rho_multiplier=4` 时降到 `3.667` | 任务 3 的连续约束容易满足，但离散修复后的选址质量会受罚函数路径影响；默认 `rho0=10, rho_multiplier=8` 速度和质量较均衡 |

这个参数实验说明：一阶算法和罚函数类算法并不是“写出来就自动好用”。它们的优势是实现简单、结构清楚，但在固定迭代预算下，参数会明显影响收敛速度、约束残差和修复后的可执行方案质量。因此在报告中应把默认参数解释为经验选择，而不是唯一最优选择。
...
- **真实场景参数敏感性仍有限**：当前已经补充了教材算法参数分析，但工位处理能力、AGV 数量、AGV 容量、缓存容量和缓存数量也会影响真实场景调度结论，后续还可以继续扩展。
