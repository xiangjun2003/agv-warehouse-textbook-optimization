...
| 任务 1 + 动态分区 + 缓存 | AGV 派车 + 工位服务区域 + 缓存点 | AGV 把货送到服务区域内的缓存点，工位再按处理速度从缓存点消化 | 缓存点或工位 |

这里的缓存不是“强制 AGV 把同一件货搬两遍”。更合理的解释是工位前缓存/中转区：AGV 负责主运输，把托盘货量补到缓存点；工位侧再通过站内短驳、人工拣选或线边补给从缓存点处理货物。因此主指标是 **所有货物处理完所需轮数** 和 **AGV 总行驶距离**；缓存到工位的站内距离会单独记录为辅助指标，不混入 AGV 总距离。
...
读取 orders.csv 与 pallets.csv，构造 t=0 初始总需求
  -> 方案 A：只滚动求解任务 1 派车
  -> 方案 B：先用任务 2 求主服务工位，再滚动求解任务 1
  -> 方案 C：在方案 B 基础上，用任务 3 选出的缓存点作为可选中转库存
...
- 12 辆 AGV 参与滚动处理，每辆每轮最多搬运 80 件；
- 18 个工位，每个工位每轮最多处理 160 件；
- 任务 3 选出 10 个缓存点，每个缓存点容量默认为 600 件。

滚动过程图展示的是第三组“任务 1 + 动态分区 + 缓存”的完整 16 轮过程：
...
- 橙色菱形：任务 3 选出的缓存点；
- 蓝色虚线：`AGV -> 托盘` 取货段；
- 橙色实线：`托盘 -> 缓存点` 的 AGV 主运输段；
- 粉色点划线：`缓存点 -> 工位` 的站内处理段。

输出文件：

```text
results/multi_period_comparison.csv  三组消融方案的完成轮数、AGV 总距离和缓存节约量
...
figures/cache_distance_comparison.png 三组消融结果比较图
...
| 多时刻滚动消融实验 | 初始总需求下的分解式滚动处理 | 第 7 章 + 第 6 章 | 任务 1 派车 LP + 任务 2 动态分区 LP + 任务 3 缓存选址 + 缓存库存滚动更新 | `solve_multi_period.py` |
...
这个结果可以看出：所有订单需求在初始时刻一次性给定，系统不是持续加货，而是每轮处理一部分剩余货量。默认参数下，仅任务 1 需要 15 轮、AGV 总距离 3503；加入动态分区后仍为 15 轮，但 AGV 总距离略降到 3494；加入缓存后，AGV 主运输距离降到 1751，比动态分区方案少 1743，节约约 49.89%，但由于缓存库存还需要由工位侧消化，完成轮数增加到 16 轮。

这个结论也解释了为什么早期“强制所有货物经过缓存，并且还让 AGV 再从缓存送到工位”的设置会变差：那等于把同一件货拆成两次 AGV 搬运，路线数和轮次都会增加。当前实验把缓存建模为工位前暂存区，AGV 只负责主运输，站内处理单独记账，因此更符合“缓存点减少 AGV 长距离行驶，但可能增加库存等待”的真实含义。
