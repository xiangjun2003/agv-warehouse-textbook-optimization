...
- $x_{ij}\in\{0,1\}$：AGV $i$ 是否服务托盘 $j$；
- $y_{jk}\in\{0,1\}$：托盘 $j$ 是否送到工位 $k$。
...
$z_{jk}\ge 0$：托盘 $j$ 分给工位 $k$ 的货量。

参数：

- $q_j$：托盘 $j$ 的总货量；
- $d_{jk}$：托盘 $j$ 到工位 $k$ 的曼哈顿距离。
...
- 每个托盘的货量必须全部分配出去：$\sum_k z_{jk}=q_j$；
- 每个工位至少获得一定货量：$\sum_j z_{jk}\ge \alpha\frac{\sum_j q_j}{K}$；
- 所有货量流非负：$z_{jk}\ge 0$。
...
- $P$：托盘集合，$|P|=140$；
- $S$：工位集合，$|S|=18$；
- $C_0$：候选缓存点集合，当前可由 140 个托盘/货位坐标构成；
- $m=10$：最终选中的缓存点数量；
- $A$：AGV 集合，$|A|=24$；
...
- $u_{ps}\in\{0,1\}$：托盘 $p$ 是否由工位 $s$ 服务；
- $v_c\in\{0,1\}$：候选缓存点 $c\in C_0$ 是否被选中；
- $x^D_{apst}\in\{0,1\}$：第 $t$ 轮 AGV $a$ 是否执行“托盘 $p$ 到工位 $s$”；
- $x^I_{apct}\in\{0,1\}$：第 $t$ 轮 AGV $a$ 是否执行“托盘 $p$ 到缓存 $c$”；
- $x^O_{acst}\in\{0,1\}$：第 $t$ 轮 AGV $a$ 是否执行“缓存 $c$ 到工位 $s$”；
- $g^D_{apst},g^I_{apct},g^O_{acst}\ge 0$：对应路线实际运输货量；
- $R_{pt}\ge 0$：第 $t$ 轮后托盘 $p$ 剩余货量；
- $I_{ct}\ge 0$：第 $t$ 轮后缓存 $c$ 的库存；
- $L_{st}\ge 0$：第 $t$ 轮后工位 $s$ 的待处理队列；
- $h_{st}\ge 0$：第 $t$ 轮工位 $s$ 实际处理货量；
...
- 托盘到工位：$140\times 18=2{,}520$；
- 托盘到缓存：$140\times 10=1{,}400$；
- 缓存到工位：$10\times 18=180$；
- 合计约 $4{,}100$ 种路线动作。
...
- 托盘到候选缓存：$140\times 140=19{,}600$；
- 缓存到工位：$140\times 18=2{,}520$；
- 仅路线动作就超过 $24{,}000$ 种；
- AGV-动作-轮次变量约为：
