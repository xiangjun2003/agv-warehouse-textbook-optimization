...
| 任务 1 + 动态分区 + 缓存 | AGV 派车 + 工位服务区域 + 缓存点 | AGV 可把货送到工位前缓存点，货物仍由对应工位处理 | 缓存点或工位 |

这里的缓存不是“货物不需要工位处理”，也不是“强制 AGV 把同一件货搬两遍”。更合理的解释是工位前缓存/中转区：AGV 负责主运输，把托盘货量送到离工位更近的缓存点；对应工位再按自己的处理速度从缓存点取货、拣选或补给。因此货物最终仍然被工位处理，只是最后一小段被视为站内处理，不计入 AGV 主运输距离。主指标是 **所有货物处理完所需轮数** 和 **AGV 总行驶距离**；缓存到工位的站内距离会单独记录为辅助指标。
...
滚动过程图展示的是第三组“任务 1 + 动态分区 + 缓存”的完整 8 轮过程：
...
这个结果可以看出：所有订单需求在初始时刻一次性给定，系统不是持续加货，而是每轮运输一部分剩余货量，同时工位以固定速度处理已经送达的队列。默认参数下，仅任务 1 选择近距离工位，AGV 总距离为 2899，但负载集中，部分工位排队，最终需要 10 轮；加入动态分区后，任务 2 给工位总负载加了均衡上限，AGV 路线距离增加到 3322，但完成时间降到 8 轮；加入缓存后，完成时间保持 8 轮，AGV 总距离降到 2445，比动态分区方案节约 877，约 26.40%。

这个结论也解释了为什么早期“强制所有货物经过缓存，并且还让 AGV 再从缓存送到工位”的设置会变差：那等于把同一件货拆成两次 AGV 搬运，路线数和轮次都会增加。当前实验把缓存建模为工位前暂存区，AGV 只负责主运输，站内处理单独记账，因此更符合“缓存点减少 AGV 长距离行驶，但不改变货物最终由工位处理”的真实含义。
