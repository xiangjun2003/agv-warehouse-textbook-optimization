...
这个仓库是一个完整的 AGV 仓储优化课程项目。项目使用本地 CSV 数据、自动化建模脚本和我们自己实现的教材优化算法，完成三个基础仓储优化任务：AGV 搬运分配、动态分区、重点缓存货位选择；并进一步构建一个初始总需求下的多时刻滚动调度消融实验。

README 按演示汇报的顺序展开：先讲背景、实体和数据，再讲基础任务与多时刻实验的实际意义、建模方式、教材章节对应算法、实验设置、结果分析、完整全局模型和实验局限性。
...
- **如果所有货物需要多轮处理，整体需要多久、走多远？** 例如一批订单总需求已经给定，AGV 每轮只能搬一次，工位每轮只能处理固定数量；此时不仅要看单次派车距离，还要看工位队列、缓存库存和完成全部货物所需轮数。这对应 **多时刻滚动调度消融实验**。
...
| --- | --- | --- | --- | --- |
| `data/map.csv` | `Type`, `X`, `Y`, `All_Car`, `Free_Car` | 704 个地图节点，仓库尺寸 32 x 22，其中类型 `5` 的工位有 18 个 | 仓库网格、通道、货位、充电桩、连接节点和工位 | 读取仓库节点与工位坐标，支撑基础任务和多时刻实验的曼哈顿距离计算 |
...
| `data/orders.csv` | `Order ID`, `SKU`, `Required Amount`, `Order Received Time`, `Deadline Time` | 675 条订单记录，181 个不同 SKU，需求总量 8478 | 订单需求背景，说明仓库为什么需要搬运和补货 | 多时刻实验将订单需求匹配到托盘库存，形成初始待处理总需求 |
...
| 多时刻滚动调度消融 | `orders.csv`, `pallets.csv`, `bots.csv`, `map.csv`，以及任务 2/3 的输出 | 将订单需求匹配到托盘，逐轮更新 AGV 位置、剩余货量、工位队列和缓存库存 |

默认实验中的随机数只用于任务 1 的 AGV 抽样。它对应现实中的“当前批次可调度车辆集合”，不是随机生成仓库、托盘或工位；仓库地图、托盘位置、托盘货量和订单数据都来自固定 CSV，因此实验可以直接复现。

## 2. 项目内容

项目包含三个基础优化任务和一个综合多时刻实验：

1. **AGV 任务分配**：决定哪辆 AGV 去搬哪个托盘，并送到哪个工位。
2. **动态分区**：决定每个托盘的货量由哪些工位处理，从而形成工位服务区域。
3. **重点缓存货位选择**：从候选货位中选出一组适合作为重点缓存或中转的位置。
4. **多时刻滚动调度消融实验**：在初始订单需求给定的情况下，比较“仅任务 1”“任务 1 + 缓存”“任务 1 + 动态分区”“任务 1 + 动态分区 + 缓存”四种方案的完成轮数和 AGV 总距离。

前三个基础任务对应仓库运行的三个层面：当天调度、区域组织和布局规划；多时刻实验把它们放到同一条处理流程中，观察每一层决策对整体完成时间和运输距离的影响。

## 3. 自动化建模与求解流程

整个项目已经自动化实现。运行 `python run_all.py` 后，程序会依次求解前三个基础任务；运行 `python solve_multi_period.py` 后，程序会构造初始订单需求并执行多时刻滚动消融实验。所有结果都会写入 `results/`。
...
| 多时刻滚动实验 | 140 个托盘、675 条订单、24 辆 AGV、18 个工位、10 个缓存点；四组消融共生成 149 到 197 条 AGV 路线，默认结果为 10 到 20 轮 |
...
`solve_multi_period.py` 当前采用的是 **分解式滚动启发式算法**，不是一次性求解完整多时刻全局最优。它的流程是：
...
如果要严谨地一次性求完整多时刻问题，可以写成一个混合整数规划。下面给出包含动态分区、缓存和 AGV 多轮调度的统一模型。
...
```text
u_ps ∈ {0,1}：托盘 p 是否由工位 s 服务
v_c  ∈ {0,1}：候选缓存点 c（c 属于 C0）是否被选中
x^D_apst ∈ {0,1}：第 t 轮 AGV a 是否执行 托盘 p -> 工位 s
x^I_apct ∈ {0,1}：第 t 轮 AGV a 是否执行 托盘 p -> 缓存 c
x^O_acst ∈ {0,1}：第 t 轮 AGV a 是否执行 缓存 c -> 工位 s
...
0 <= h_st <= H
0 <= h_st <= L_s,t-1 + 当轮送达量

完成条件：
F_t = 1 只有当所有 R_p,t、I_c,t、L_s,t 都为 0 时才能成立
```
...
简单任务分配变量：24 * 197 * 20 ≈ 94,560 个 0-1 变量
```

但如果要严格考虑 AGV 上一轮终点会影响下一轮空驶距离，就需要路线转移变量：

```text
24 * 19 * 197^2 ≈ 17,700,000 个 0-1 转移变量
```
...
```text
24 * 4,100 * 20 ≈ 1,968,000 个 0-1 变量
```

如果连缓存选址也一起放入同一个模型，候选缓存点不是 10 个，而是最多 140 个候选货位，此时 `托盘 -> 缓存` 动作会变为：

```text
托盘 -> 候选缓存：140 * 140 = 19,600
缓存 -> 工位：140 * 18 = 2,520
仅路线动作就超过 24,000 种
AGV-动作-轮次变量约为 24 * 24,640 * 20 ≈ 11,827,200 个 0-1 变量
```
...
- **不是全局最优**：多时刻实验是滚动启发式，不保证得到完整多时刻问题的最优轮数或最小总距离。
