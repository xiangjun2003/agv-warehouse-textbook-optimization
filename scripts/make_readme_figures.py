...
def draw_background_overview() -> None:
    ...
    add_metric(ax_info, 0.86, "地图节点（通道/货位/工位等）", f"{len(data.nodes)}", "#1d1d1f")
    add_metric(ax_info, 0.70, "托盘候选位置", f"{len(data.pallets)}", "#34c759")
    add_metric(ax_info, 0.54, "实验采样 AGV", f"{len(data.agvs)}", "#007aff")
    add_metric(ax_info, 0.38, "工位", f"{len(data.workstations)}", "#ff3b30")
    add_metric(ax_info, 0.22, "订单记录（业务需求背景）", f"{len(orders)}", "#af52de")
    ax_info.text(
        0.08,
        0.08,
...
def draw_task1_assignment() -> None:
    ...
    workstations = np.asarray(data.workstations)
    agvs = np.asarray(data.agvs)
    ax.scatter(pallets[:, 0], pallets[:, 1], s=20, c="#c7f0d2", alpha=0.75, label="未选托盘")
    ax.scatter(workstations[:, 0], workstations[:, 1], s=130, c="#ff3b30", marker="s", edgecolors="white", label="工位")
    ax.scatter(agvs[:, 0], agvs[:, 1], s=130, c="#007aff", marker="^", edgecolors="white", label="AGV")

    route_colors = plt.cm.tab10(np.linspace(0, 1, min(10, len(assignments))))
    shown = assignments.head(10)
    for color, row in zip(route_colors, shown.itertuples(index=False)):
        ...
        ax.scatter([pallet[0]], [pallet[1]], s=90, c=[color], edgecolors="white", linewidths=0.9)
    ...
    ax.text(
        ...
        fontsize=11,
        color="#3a3a3c",
        bbox={"boxstyle": "round,pad=0.45", "facecolor": "white", "edgecolor": "#d2d2d7", "alpha": 0.94},
    )
...
def draw_task2_partition() -> None:
    ...
    cmap = plt.get_cmap("tab20", max(len(workstations), 1))
    colors = [cmap(idx % 20) if idx >= 0 else "#c7c7cc" for idx in station_for_pallet]
    ...
    ax_map.scatter(workstations[:, 0], workstations[:, 1], s=150, c="#1d1d1f", marker="s", edgecolors="white", linewidths=1.0, label="工位")
    for idx, (x, y) in enumerate(data.workstations):
        ax_map.text(x + 0.24, y + 0.24, str(idx), fontsize=8, color="#1d1d1f", weight="bold")
    setup_map_axis(ax_map, data)
    ax_map.set_title("任务 2：托盘货量按主服务工位形成动态分区", fontsize=18, fontweight="bold", pad=12)
    ax_map.legend(loc="upper left", frameon=True, facecolor="white", framealpha=0.95)

    loads = loads.sort_values("workstation_index")
    ax_bar.barh(loads["workstation_index"].astype(str), loads["quantity"], color="#30d158", alpha=0.82)
...
def draw_task3_layout() -> None:
    ...
    scatter_nodes(ax_map, data, alpha=0.10)
    ax_map.scatter(pallets[:, 0], pallets[:, 1], s=34, c="#d7ecff", alpha=0.78, label="候选货位")
    ax_map.scatter(
        selected[:, 0],
        selected[:, 1],
        s=180,
    ...
    for x, y in selected:
        ...
        ax_map.plot(diamond_x, diamond_y, color="#ff9f0a", alpha=0.24, linewidth=1.2)
    setup_map_axis(ax_map, data)
